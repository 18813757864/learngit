...
var LunarHelp = function () {
    function LunarHelp(year, month, day) {
        ...
        var date = new Date(parseInt(year), parseInt(month) - 1, parseInt(day)).getTime();

        var i,
            leap = 0,
            temp = 0; //天数
        // var baseDate = new Date(1900, 0, 31).getTime();
        var baseDate ="-2206425943000"; //不要new Date创建
...
// 日历的构造函数，执行_createClass函数为SimpleCalendar函数的原型对象赋值。 this._options为叠加后的配置对象。
var SimpleCalendar = function () {
    ...
    _createClass(SimpleCalendar,
        [
            ...
            //刷新年份和月份下拉框 只有在初始化和设置语言后才会更新key: 'updateSelect',
            {
                key: 'updateSelect',
            ...
            //刷新标记日期key: 'updateMark',(0,"待审核"),(1,"通过"),(2,"不通过"),(3,"未提交")
            // 根据需要自己进行添加想要的类。
            {
                key: 'updateMark',
                value: function updateMark(year, month,targetDay,info) {
                    ...
                    if (options.showMark) {
                        ...
                        if(info){
                            ...
                            switch (info.inputState){
                                case 2: markTemp = "sc-blue";
                                        titleString = "已录菜谱";
                                        break;
                                case 1: markTemp = "sc-white";
                                        titleString = "未录菜谱";
                                        break;
                                default: markTemp="sc-gray";
                                        titleString = "未开餐";
                                    break;
                            }
                        }
                        ...
                        daysElement.forEach(function (v, i) {
                            ...
                            data = options.hasWriteMark;
                            if(parseInt(day)<10){
                                day = "0"+day;
                            }
                            // 更新页面，添加数据
                            if (data[year + '-' + currentmonth + '-' + targetDay]&&(day==targetDay)) {
                                v.dataset.seq = JSON.stringify(info)||"";
                                v.classList.add(markTemp);
                                v.title = titleString;
                            }else if(data[year + '-' + currentmonth + '-' + targetDay]&&(day==targetDay)){
                                v.dataset.seq = JSON.stringify(info)||"";
                                v.classList.remove("sc-blue","sc-gray");
                                v.title = '';
                            } else if(!data[year + '-' + currentmonth + '-' + day]){
                                 v.classList.remove("sc-blue","sc-white");
                                v.dataset.seq="";
                                v.title = '';
                            }
                ...
                value: function updateEvent() {
                    ...
                    daysElement.forEach(function (v, i) {
                        ...
                        v.onclick = function (event) {
                            ...
                            var classList = _this.selectDay.className;
                            var infoFlag = 0;// 未填写
                            if(classList.indexOf("sc-hasWriteMark")>=0){
                                infoFlag = 1;//已填写
                            }
                            if (classList.indexOf("sc-othermenth") > 0) {
                                infoFlag = 999;//不属于本月,不用填写
                            }
                            // //WAIT(0,"待审核"),PASS(1,"通过"),NOT_PASS(2,"不通过"),NOT_COMMIT(3,"未提交");
                            // if (testssss.indexOf("sc-mark") > 0) {
                            //     infoFlag = 1;//通过
                            // } else if (testssss.indexOf("sc-waitMark") > 0) {
                            //     infoFlag = 0;//待审核
                            // } else if (testssss.indexOf("sc-unPassMark") > 0) {
                            //     infoFlag = 2;//未通过
                            // } else if (testssss.indexOf("sc-unCommitMark") > 0) {
                            //     infoFlag = 3;//填写未提交
                            // } else if (testssss.indexOf("sc-othermenth") > 0) {
                            //     infoFlag = 999;//不属于本月,不用填写
                            // } else {
                            //     infoFlag = 998;//未填写
                            // }
                            this.classList.add('sc-selected');
                            _this.getSelectedDay(infoFlag,this);
                        };
                    });

                    var selectYear = container.querySelector('.sc-select-year');
                    var selectMonth = container.querySelector('.sc-select-month');
                    var selectYearValue = selectYear.value;
                    var selectMonthValue = selectMonth.value;
                    selectYear.onchange = function () {
                        var y = this.value;
                        updateCalendar(y,selectMonthValue,0);
                    };

                    selectMonth.onchange = function () {
                        var m = this.value;
                        updateCalendar(selectYearValue,m,0);
                    };
                    ...
                    yearadd.onclick = function () {
                        if (selectYearValue < 2099) selectYearValue++;
                        selectYear.value = selectYearValue;
                        updateCalendar(selectYearValue,selectMonthValue);
                    };
                    yearsub.onclick = function () {
                        if (selectYearValue > 1900) selectYearValue--;
                        selectYear.value = selectYearValue;
                        updateCalendar(selectYearValue,selectMonthValue);
                    };
                    monthadd.onclick = function () {
                        if (selectMonthValue < 12) selectMonthValue++; else {
                            selectMonthValue = 1;
                            selectYear.value = ++selectYearValue;
                        }
                        ;
                        selectMonth.value = selectMonthValue;
                        updateCalendar(selectYearValue,selectMonthValue);
                    };
                    monthsub.onclick = function () {
                        if (selectMonthValue > 1) selectMonthValue--; else {
                            selectMonthValue = 12;
                            selectYear.value = --selectYearValue;
                        }
                        selectMonth.value = selectMonthValue;
                    ...
                    returntoday.onclick = function () {
                        selectYear.value = _this.tyear;
                        selectMonth.value = _this.tmonth;
                        updateCalendar(_this.tyear,_this.tmonth);
                    };

                    //根据选择的日期更新日历，触发配种中的selectMonthOrYear函数
                    function updateCalendar(year,month,type) {
                        _this._options.hasWriteMark = {};
                        _this.update(month, year);
                        //获取选取月份每天的填写状态
                        var now = new Date();
                        var nowyear = now.getFullYear();
                        var nowday = now.getDate();
                        var nowmonth = now.getMonth() + 1;
                        // 大于今年月份以及大于今年的不需要添加额外的样式
                        if ((year == nowyear && month <= nowmonth) || year < nowyear) {
                            // 触发配置中的selectMonthOrYear函数
                            if (month < 10) {
                                _this._options.selectMonthOrYear(year + "-0" + month);
                            } else {
                                _this._options.selectMonthOrYear(year + "-" + month);
                            }
                        }
            ...
            //添加标记key: 'addMark',(0,"待审核"),(1,"通过"),(2,"不通过"),(3,"未提交")
            // 可以调用该方法标注哪些日期为哪些状态。
            // 调用updateMark去更新样式。
            {
                key: 'addMark',
                value: function addMark(day, info) {
                        this._options.hasWriteMark[day] = "已填写";
                        this.updateMark(day.split("-")[0], day.split("-")[1], day.split("-")[2],info);
                }
            },
            //获取用户点击的日期key: 'getSelectedDay',
            // 插件默认的点击事件。
            {
                key: 'getSelectedDay',
                value: function getSelectedDay(infoFlag,dom) {
                    ...
                    var dateString = selectYear + "-" + selectMonth + "-" + selectDay;
                    this._options.getClickDate(infoFlag,dateString,dom);
            ...
            */
            ...
            //设置是否显示阴历节日key: 'showLunarFestival'
            {
            ...
            //设置是否显示阴历日期key: 'showLunarCalendar'
            {
            ...
            //将nodelist转为数组key: 'arrayfrom',方便代码中使用
            //nodelist转数组
            {
                key: 'arrayfrom',
